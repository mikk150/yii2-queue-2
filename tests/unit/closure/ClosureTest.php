...
class ClosureTest extends TestCase
{
    public function testPush1()
    {
        $this->getQueue()->push(function () {
            $fileName = Yii::getAlias(codecept_output_dir() . 'job-1.lock');
            file_put_contents($fileName, '');
        });
        $this->getQueue()->run();
        $this->assertFileExists(Yii::getAlias(codecept_output_dir() . 'job-1.lock'));
    }

    public function testPush2()
    {
        $fileName = Yii::getAlias(codecept_output_dir() . 'job-2.lock');
